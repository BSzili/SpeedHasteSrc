...
* Initializes PS3M. Allocates one AHI sound for the module.
* In:
*    d0 = module length
*    d1 = AHI sound id to be used for AHI_LoadSound()
*    d2 = First AHI channel to be used for the module.
*    a0 = Address of the module (S3M, MOD, XM, MTM)
*    a1 = AHIBase
*    a2 = AHICtrl 
*    a3 = location for AHI play music callback address
*    a4 = location for AHI soundfunc callback address
* Out:
*    d0 = True if ok, false otherwise
;*    a0 = AHI play music callback address
;*    a1 = AHI soundfunc callback address
	XDEF    _ps3mInitialize
_ps3mInitialize:
	movem.l	d2-d7/a2-a6,-(sp)
    move.l  d0,setmodulelen
    move.l  d1,AHIModuleSoundId
    move.l  d2,AHIFirstChannel
    move.l  a0,moduleAddress
    move.l  a0,setmodule
    move.l  a1,AHIBase
    move.l  a2,AHICtrl

    * Assume samples are not yet converted:
    clr     fformat

    move    #0,ahi_stereolev
    st      ahi_use
    ...
    ;lea     ahi_playmusic(pc),a0
    ;lea     soundfunc(pc),a1
	move.l #ahi_playmusic,(a3)
	move.l #soundfunc,(a4)
...
* Stop playback by silencing the music AHI channels
* and stopping music playback.
	XDEF    _ps3mStop
_ps3mStop:
    bra    ahi_stop
...
* Uninitialize PS3M, unloads the AHI sound previously loaded.
	XDEF    _ps3mUninitialize
_ps3mUninitialize:
;    bra     ahi_end
	movem.l	d2-d7/a2-a6,-(sp)
	bsr     ahi_end
...
* Out:
*   d0 = true if playing, false otherwise
	XDEF    _ps3mIsPlaying
_ps3mIsPlaying:
    tst.b   setpause
    beq     .paused
...
* Out:
*    d0 = Current song position
;*    d1 = Max song position
	XDEF    _ps3mGetSongPosition
_ps3mGetSongPosition:
    moveq   #0,d0
...
* Out:
*    d0 = Current row position
	XDEF    _ps3mGetRowPosition
_ps3mGetRowPosition:
    moveq   #0,d0
    move    rows,d0
    rts
...
* In:
*    d0 = New song position to set
*    d1 = New row position to set (S3M only)
	XDEF    _ps3mSetSongAndRowPosition
_ps3mSetSongAndRowPosition:
;    bra    setPosition2
	movem.l	d2-d7/a2-a6,-(sp)
	bsr    setPosition2
	movem.l	(sp)+,d2-d7/a2-a6
...
AHIBase:          dc.l    0
AHICtrl:          dc.l    0
AHIModuleSoundId: dc.l    0
AHIFirstChannel:  dc.l    0
...
TESTMODE
	...
	* cybercalib: false
	moveq	#0,d0       * cyber calib: false    
	moveq	#0,d1       * cyber calib table addr
	moveq	#1,d2       * ahi_use: tru
	move.l	#24000,d3       * ahi_rate: mixiing rate
	move.l	#63,d4       * ahi_mastervol
	moveq	#0,D5       * ahi_stereolev
	move.l	#$20009,D6       * ahi_mode: paula stereo
...
.fr
 ...
 ifeq TEST
	...
	jsr	(a5)			* kanavien m��r�st� ja tunen nimest�
...
;	move.l	d0,a2
	move.l  AHIModuleSoundId,d0				;Load module as one sound!
...
ahi_sample:
	...
	move.l	AHIModuleSoundId,d1				;sample bank
...
; Convert sample
    ...
    * ensure conversion is only done once for this sample
    clr.b   instype(a1)
...
.eik
	...
	st	31(a0)		* asetetaan byte merkiks samplek��nn�ksest�!
